  ...
  return (
    <div className={enabled ? 'flex-1 min-w-0' : 'hidden'} data-testid="google-signin">
      ...
      <div className="group relative h-12 w-full">
        ...
        <div className="absolute inset-0 flex items-center justify-center gap-3 rounded-xl text-sm font-semibold transition-colors border border-border bg-card text-foreground group-hover:bg-secondary dark:border-white/10 dark:bg-[#1e1f22] dark:text-white dark:group-hover:bg-[#2a2b2f]">
      ...
      <Dialog open={!!pendingCredential} onOpenChange={(open) => { if (!open) setPendingCredential(''); }}>
        {/* El dialogo vive en un portal fuera del arbol del login, y sigue el
            tema activo del sitio (monocromo en los dos). */}
        <DialogContent className="max-w-md bg-popover dark:bg-[#0d0d0f] border-border dark:border-white/10 text-foreground" data-testid="google-consent-dialog">
