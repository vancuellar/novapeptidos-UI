...
import { Store, User, PlayCircle, TrendingUp, Bell, Ticket, Users, Truck, ShoppingBag, FileText, Calculator, Coins, Package } from 'lucide-react';
...
// "Tutoriales": videos cortos y guías escritas.
//
// Vive aquí —y no dentro de la página /tutoriales— porque desde el 2026-07-30
// es UNA PESTAÑA MÁS del menú de cada tablero (el del cliente y el del
// distribuidor). La página suelta seguía existiendo en paralelo y era justo la
// información regada que Christián pidió juntar: ahora /tutoriales sólo recibe
// a los visitantes sin cuenta y manda a los miembros a su propia pestaña.
...
const TutorialsPanel = () => {
  ...
  const { user } = useAuth();
  // La guía y los videos de distribuidor sólo se muestran a distribuidores/admin.
  ...
  return (
    <div data-testid="tutorials-panel">
      <p className="text-muted-foreground text-sm mb-6">{t('tutorials.subtitle')}</p>
