...
const ProductCard = ({ product }) => {
  ...
  return (
    <Card data-testid="product-card" className="group overflow-hidden border-border bg-white shadow-[var(--shadow-sm)] hover:shadow-[var(--shadow-md)] transition-shadow duration-200 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex flex-wrap gap-1.5 mb-2">
          ...
          <Badge variant="outline" className="text-[10px] border-amber-200 bg-amber-50 text-amber-900" data-testid="product-card-ruo-badge">RUO</Badge>
          {product.is_new && <Badge className="text-[10px] bg-[hsl(var(--info))] bg-sky-600">Nuevo</Badge>}
        </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          {out ? <span className="text-xs text-muted-foreground">Agotado</span> : <span className="text-xs text-[hsl(var(--success))] text-emerald-700">En stock</span>}
        </div>
