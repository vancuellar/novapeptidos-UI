      ...
      <Link to={`/producto/${product.slug}`} className="block relative">
        <div className="bg-[hsl(var(--secondary))] overflow-hidden">
          <AspectRatio ratio={4 / 3}>
            <img src={productImage(product, active, true)} alt={product.name} className="h-full w-full object-cover transition-transform duration-300 ease-out hover:scale-110" loading="lazy" decoding="async" />
          </AspectRatio>
        </div>
        {product.purity && (
