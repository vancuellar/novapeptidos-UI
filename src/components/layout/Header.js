    ...
    <header className="sticky top-0 z-40">
      ...
      <div className="bg-[hsl(var(--secondary))] text-[hsl(var(--muted-foreground))] border-b border-border text-xs">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-9 flex items-center justify-center sm:justify-between gap-4">
          <div className="hidden sm:flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5 text-[hsl(var(--primary))]" /> COA por lote disponible</div>
          <div className="flex items-center gap-2"><Truck className="h-3.5 w-3.5 text-[hsl(var(--primary))]" /> Envío nacional en México · Gratis desde $2,500 MXN</div>
          <div className="hidden sm:flex items-center gap-2 font-mono-tech">Solo para uso en investigación (RUO)</div>
        </div>
      </div>
      ...
      <div className="bg-white/80 supports-[backdrop-filter]:backdrop-blur-md border-b border-border">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-4">
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="left" className="w-80">
              ...
              <form onSubmit={submitSearch} className="mt-4">
                <Input placeholder="Buscar péptidos..." value={search} onChange={(e) => setSearch(e.target.value)} data-testid="site-search-input-mobile" />
              </form>
              <nav className="mt-6 flex flex-col gap-1">
                <Link to="/catalogo" onClick={() => setMobileOpen(false)} className="py-2 font-medium">Todo el catálogo</Link>
                {categories.map((c) => (
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0" data-testid="header-logo">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[10px] text-muted-foreground -mt-1 hidden sm:block">Péptidos de investigación · México</div>
            </div>
          </Link>
          ...
          <form onSubmit={submitSearch} className="hidden md:flex flex-1 max-w-xl mx-auto">
            <div className="relative w-full">
              ...
              <Input className="pl-9" placeholder="Buscar BPC-157, Ipamorelin, TB-500..." value={search} onChange={(e) => setSearch(e.target.value)} data-testid="site-search-input" />
            </div>
          </form>
            ...
            {user ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-52">
                  ...
                  <DropdownMenuItem onClick={() => navigate('/cuenta')}><User className="h-4 w-4 mr-2" /> Mi cuenta</DropdownMenuItem>
                  {user.role === 'admin' && (
                    <DropdownMenuItem onClick={() => navigate('/admin')} data-testid="header-admin-link"><LayoutDashboard className="h-4 w-4 mr-2" /> Panel admin</DropdownMenuItem>
                  )}
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => { logout(); navigate('/'); }} data-testid="header-logout-button"><LogOut className="h-4 w-4 mr-2" /> Cerrar sesión</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ) : (
        ...
        <div className="hidden lg:block border-t border-border">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-11 flex items-center gap-6 overflow-x-auto">
            <Link to="/catalogo" className="text-sm font-medium whitespace-nowrap hover:text-[hsl(var(--primary))]">Todo</Link>
            {categories.map((c) => (
