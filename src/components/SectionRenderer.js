...
import { ChevronRight, Check, AlertTriangle, Info, Search, Ban, ArrowRight } from 'lucide-react';

// Renderizador de secciones tipadas (prose, list, steps, table, faq, glossary,
// callout, cards). Nació en LearnPage; se extrajo aquí para que las páginas de
// políticas y soporte (/info/*) usen exactamente el mismo lenguaje visual.
...
// Tarjeta de enlace: `to` para rutas internas, `href` para WhatsApp/correo.
const Cards = ({ section }) => (
  <>
    {section.intro && <p className="text-sm text-muted-foreground mb-3">{section.intro}</p>}
    <div className="grid sm:grid-cols-2 gap-3">
      {section.items.map((c) => {
        ...
        const cls = 'rounded-xl border border-[hsl(var(--border))] p-4 hover:border-[hsl(var(--primary))] transition-colors';
        return c.href ? (
          <a key={c.href} href={c.href} target={c.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer" className={cls}>{inner}</a>
        ) : (
          <Link key={c.to} to={c.to} className={cls}>{inner}</Link>
        );
      })}
    </div>
  </>
...
export const RENDERERS = { prose: Prose, list: ListBlock, steps: Steps, table: TableBlock, faq: Faq, glossary: Glossary, callout: Callout, cards: Cards };

export const anchorId = (title) =>
  title.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

// Cuerpo completo: índice lateral (si hay ≥4 secciones con título) + secciones.
const SectionRenderer = ({ sections, showToc: forceToc }) => {
  const titled = sections.filter((s) => s.title);
  const showToc = forceToc !== undefined ? forceToc : titled.length >= 4;

  return (
    <div className={showToc ? 'grid lg:grid-cols-[220px_1fr] gap-8 items-start' : ''}>
      {showToc && (
        <nav className="hidden lg:block sticky top-24" data-testid="page-toc">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">En esta página</div>
          <ul className="space-y-1.5">
            {titled.map((s) => (
              <li key={s.title}>
                <a href={`#${anchorId(s.title)}`} className="text-sm text-muted-foreground hover:text-[hsl(var(--primary))] transition-colors block leading-snug">{s.title}</a>
              </li>
            ))}
          </ul>
        </nav>
      ...
      <div className="space-y-8 min-w-0">
        {sections.map((section, i) => {
          ...
          return (
            <section key={i} id={section.title ? anchorId(section.title) : undefined} className="scroll-mt-24">
